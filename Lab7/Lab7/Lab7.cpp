...
int main()
{
	...
	Vector<int>* firstVector = new Vector<int>();
	...
	Vector<int>* secondVector = new Vector<int>();
	for (int i = 0; i < secondVectorLength; i++) {
		string message = "[" + to_string(i) + "] = ";
		secondVector->push_back(GetValueFromConsole::GetInt(message));
	}

	Matrix<int>* matrix = new Matrix<int>(*firstVector, *secondVector);

	matrix->Print("Matrix");

	Matrix<int>* m = (*firstVector) + (*secondVector);
