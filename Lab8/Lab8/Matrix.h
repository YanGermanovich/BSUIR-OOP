...
{
	this->rowsCount = obj.rowsCount;
	this->columnsCount = obj.columnsCount;
	this->matrix = new T*[this->rowsCount];

	for (int i = 0; i < this->rowsCount; i++) {
		this->matrix[i] = new T[this->columnsCount];
...
template <class T>
void Matrix<T>::ReplaceAll(T value) {
	for (int i = 0; i < this->rowsCount; i++) {
		for (int j = 0; j < this->columnsCount; j++) {
